document.addEventListener("DOMContentLoaded", function () {
  ...
  async function carregarFavoritos() {
    ...
    try {
      const response = await fetch(
        `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.FAVORITOS.LISTAR}`,
        {
          headers: {
  ...
  function renderizarFavoritos(favoritos) {
    ...
    favoritos.forEach((favorito) => {
      ...
      card.innerHTML = `
            ...
            <a href="/produto?id=${
              favorito.produto.id
                ...
                <img class="favorito-imagem" src="${
                  produto.imagem_url || "../static/imagens_icons/no-image.png"
                }" 
                    alt="${produto.nome}">
                <h2 class="favorito-nome">${produto.nome}</h2>
                <p class="favorito-preco">R$ ${parseFloat(
                  produto.preco
                ).toFixed(2)}</p>
                <p class="favorito-parcelamento">2x R$ ${(
                  parseFloat(produto.preco) / 2
                ).toFixed(2)} sem juros</p>
      ...
      const btnAdicionarCarrinho = card.querySelector(
  ...
  async function removerFavorito(id) {
    try {
      const response = await fetch(
        `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.FAVORITOS.REMOVER}${id}/`,
        {
          method: "DELETE",
          headers: {
  ...
  async function adicionarAoCarrinho(produto) {
    try {
      const response = await fetch(
        `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.CARRINHO.BASE}`,
        {
          method: "POST",
          headers: {
